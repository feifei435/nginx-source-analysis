...
ngx_thread_volatile ngx_event_t  *ngx_posted_accept_events;				//	accept事件队列，当有此标记时（NGX_POST_EVENTS）需要将accept延迟处理，而放到此队列中
ngx_thread_volatile ngx_event_t  *ngx_posted_events;					//	正常的读写事件队列（同ngx_posted_accept_events）
